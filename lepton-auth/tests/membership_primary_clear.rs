...
async fn seed_user(valence: &valence::Valence) -> RecordId {
    ...
    .expect("user");
    User::create_used(user, valence, valence::use_!("create User in lepton-auth/tests/membership_primary_clear.rs; Valence persistence for this feature path; typed store; visible to test harness."))
        .await
        .expect("create")
        .id()
        .cloned()
...
async fn seed_account(valence: &valence::Valence, name: &str, user: &RecordId) -> RecordId {
    ...
    .expect("account");
    Account::create_used(account, valence, valence::use_!("create Account in lepton-auth/tests/membership_primary_clear.rs; Valence persistence for this feature path; typed store; visible to test harness."))
        .await
        .expect("create")
        .id()
        .cloned()
...
async fn seed_membership(
    ...
    .expect("m");
    AccountMembership::create_used(m, valence, valence::use_!("create AccountMembership in lepton-auth/tests/membership_primary_clear.rs; Valence persistence for this feature path; typed store; visible to test harness."))
        .await
        .expect("create")
        .id()
        .cloned()
...
async fn membership_delete_clears_matching_email_primary_happy() {
    ...
    .expect("email");
    let owner_email = AccountEmail::create_used(owner_email, &valence, valence::use_!("create AccountEmail in lepton-auth/tests/membership_primary_clear.rs; Valence persistence for this feature path; typed store; visible to test harness."))
        .await
        .expect("create");
    ...
    .expect("email");
    let persona_email = AccountEmail::create_used(persona_email, &valence, valence::use_!("create AccountEmail in lepton-auth/tests/membership_primary_clear.rs; Valence persistence for this feature path; typed store; visible to test harness."))
        .await
        .expect("create");
    ...
    Account::get_used(&bare_id_from_record(&account), &valence, valence::use_!("get Account in lepton-auth/tests/membership_primary_clear.rs; Valence persistence for this feature path; typed store; visible to test harness."))
        .await
        .expect("get")
        .expect("a")
        .get_mutable_used(&valence, valence::use_!("get_mutable via membership_primary_clear.rs; mutable handle for in-place update; typed store; session/service path."))
    ...
    User::get_used(&bare_id_from_record(&persona), &valence, valence::use_!("get User in lepton-auth/tests/membership_primary_clear.rs; Valence persistence for this feature path; typed store; visible to test harness."))
        .await
        .expect("get")
        .expect("u")
        .get_mutable_used(&valence, valence::use_!("get_mutable via membership_primary_clear.rs; mutable handle for in-place update; typed store; session/service path."))
    ...
    User::get_used(&bare_id_from_record(&owner), &valence, valence::use_!("get User in lepton-auth/tests/membership_primary_clear.rs; Valence persistence for this feature path; typed store; visible to test harness."))
        .await
        .expect("get")
        .expect("u")
        .get_mutable_used(&valence, valence::use_!("get_mutable via membership_primary_clear.rs; mutable handle for in-place update; typed store; session/service path."))
    ...
    let acct = Account::get_used(&bare_id_from_record(&account), &valence, valence::use_!("get Account in lepton-auth/tests/membership_primary_clear.rs; Valence persistence for this feature path; typed store; visible to test harness."))
        .await
        .expect("get")
        .expect("account");
...
async fn membership_delete_se_failure_still_deletes() {
    ...
    assert!(
        AccountMembership::get_used(&bare_id_from_record(&persona_m), &valence, valence::use_!("get AccountMembership in lepton-auth/tests/membership_primary_clear.rs; Valence persistence for this feature path; typed store; visible to test harness."))
            .await
            .expect("get")
            .is_none()
    );
