...
async fn seed_account(valence: &valence::Valence, name: &str, user: &RecordId) -> RecordId {
    ...
    .expect("account");
    let created = Account::create(account, valence, valence::use_!(r"**Test:** Fixture **Account** save for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only."))
        .await
...
async fn seed_membership_with_role(
    ...
    let created = AccountMembership::create(membership, valence, valence::use_!(r"**Test:** Fixture **Account Membership** save for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only."))
        .await
...
async fn seed_enabled_totp(valence: &valence::Valence, user: &RecordId) -> String {
    ...
    .expect("totp factor");
    TotpFactor::upsert(&factor_id, factor, valence, valence::use_!(r"**Test:** Fixture **Totp Factor** save for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only."))
        .await
        .expect("upsert totp");
    factor_id
}
...
async fn seed_account_email(
    ...
    let email = AccountEmail::create(email_row, valence, valence::use_!(r"**Test:** Fixture **Account Email** save for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only."))
        .await
        .expect("create email");
...
async fn account_primary_email_optional_none_and_record_fk() {
    ...
    let loaded = Account::get(&account_bare, &valence, valence::use_!(r"**Test:** Fixture **Account** load for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only."))
        .await
        .expect("get")
        .expect("account");
    ...
    User::get(&bare_id_from_record(&user), &valence, valence::use_!(r"**Test:** Fixture **User** load for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only."))
        .await
        .expect("get user")
        .expect("user")
        .get_mutable(&valence, valence::use_!(r"**Test:** Fixture **this data** access in `account_primary_email` so the suite can arrange and assert persistence. CI and developers running the suite only."))
    ...
    Account::get(&account_bare, &valence, valence::use_!(r"**Test:** Fixture **Account** load for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only."))
        .await
        .expect("get account")
        .expect("account")
        .get_mutable(&valence, valence::use_!(r"**Test:** Fixture **this data** access in `account_primary_email` so the suite can arrange and assert persistence. CI and developers running the suite only."))
    ...
    let user_row = User::get(&bare_id_from_record(&user), &valence, valence::use_!(r"**Test:** Fixture **User** load for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only."))
        .await
        .expect("reload user")
        .expect("user");
    let account_row = Account::get(&account_bare, &valence, valence::use_!(r"**Test:** Fixture **Account** load for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only."))
        .await
...
async fn set_account_primary_email_happy() {
    ...
    let account_row = Account::get(&bare_id_from_record(&account), &valence, valence::use_!(r"**Test:** Fixture **Account** load for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only."))
        .await
        .expect("get")
        .expect("account");
    assert_eq!(account_row.primary_email(), Some(&email_id));
...
async fn login_resolves_user_via_primary_email_fk() {
    ...
    let email_row = AccountEmail::query(&valence, valence::use_!(r"**Test:** Fixture **Account Email** list for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only."))
        .where_address(StringPredicate::Equals("login@example.test".into()))
        .first()
        .await
        .expect("query")
        .expect("email");
    let found = GeneratedUser::query(&valence, valence::use_!(r"**Test:** Fixture **Generated User** list for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only."))
...
async fn delete_primary_email_restricted() {
    ...
    Account::get(&bare_id_from_record(&account), &valence, valence::use_!(r"**Test:** Fixture **Account** load for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only."))
        .await
        .expect("get")
        .expect("account")
        .get_mutable(&valence, valence::use_!(r"**Test:** Fixture **this data** access in `account_primary_email` so the suite can arrange and assert persistence. CI and developers running the suite only."))
    ...
    assert!(AccountEmail::get(&bare_id_from_record(&email_id), &valence, valence::use_!(r"**Test:** Fixture **Account Email** load for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only."))
        .await
        .expect("get")
        .is_some());
}
...
async fn sole_member_user_delete_denied() {
    ...
    assert!(User::get(&bare_id_from_record(&user), &valence, valence::use_!(r"**Test:** Fixture **User** load for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only."))
        .await
        .expect("get")
        .is_some());
}
...
async fn erase_account_cascades_emails_and_deletes_users() {
    ...
    assert!(Account::get(&bare_id_from_record(&account), &valence, valence::use_!(r"**Test:** Fixture **Account** load for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only."))
        .await
        .expect("get")
        .is_none());
    assert!(User::get(&bare_id_from_record(&user), &valence, valence::use_!(r"**Test:** Fixture **User** load for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only."))
    ...
    assert!(AccountEmail::get(&bare_id_from_record(&email_id), &valence, valence::use_!(r"**Test:** Fixture **Account Email** load for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only."))
        ...
        .expect("get")
        .is_none());
    assert!(AccountEmail::get(&bare_id_from_record(&backup), &valence, valence::use_!(r"**Test:** Fixture **Account Email** load for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only."))
        .await
        .expect("get")
        .is_none());
    assert!(TotpFactor::get(&totp_id, &valence, valence::use_!(r"**Test:** Fixture **Totp Factor** load for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only."))
        .await
        .expect("get totp")
        .is_none());
}
...
async fn erase_account_deletes_all_member_users() {
    ...
    assert!(Account::get(&bare_id_from_record(&account), &valence, valence::use_!(r"**Test:** Fixture **Account** load for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only."))
        ...
        .expect("get")
        .is_none());
    assert!(User::get(&bare_id_from_record(&owner), &valence, valence::use_!(r"**Test:** Fixture **User** load for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only."))
        .await
        .expect("get")
        .is_none());
    assert!(User::get(&bare_id_from_record(&persona), &valence, valence::use_!(r"**Test:** Fixture **User** load for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only."))
        .await
        .expect("get")
        .is_none());
}
...
async fn persona_delete_denied_when_account_primary() {
    ...
    User::get(&bare_id_from_record(&persona), &valence, valence::use_!(r"**Test:** Fixture **User** load for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only."))
        .await
        .expect("get")
        .expect("user")
        .get_mutable(&valence, valence::use_!(r"**Test:** Fixture **this data** access in `account_primary_email` so the suite can arrange and assert persistence. CI and developers running the suite only."))
    ...
    User::get(&bare_id_from_record(&owner), &valence, valence::use_!(r"**Test:** Fixture **User** load for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only."))
        .await
        .expect("get")
        .expect("user")
        .get_mutable(&valence, valence::use_!(r"**Test:** Fixture **this data** access in `account_primary_email` so the suite can arrange and assert persistence. CI and developers running the suite only."))
...
async fn persona_user_delete_allowed_when_sibling_exists() {
    ...
    assert!(User::get(&bare_id_from_record(&owner), &valence, valence::use_!(r"**Test:** Fixture **User** load for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only."))
        ...
        .expect("get")
        .is_some());
    assert!(AccountEmail::get(&bare_id_from_record(&login), &valence, valence::use_!(r"**Test:** Fixture **Account Email** load for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only."))
        .await
        .expect("get")
        .is_some());
}
...
async fn account_email_provision_signup_happy() {
    ...
    let user = User::get(&bare_id_from_record(&pending.user_id), &valence, valence::use_!(r"**Test:** Fixture **User** load for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only."))
        .await
        .expect("get")
        .expect("user");
    let account = Account::query(&valence, valence::use_!(r"**Test:** Fixture **Account** list for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only."))
        .where_user(RecordPredicate::Equals(pending.user_id.clone()))
        .first()
        .await
        .expect("query")
        .expect("account");
    let email = AccountEmail::get(&bare_id_from_record(&pending.email_id), &valence, valence::use_!(r"**Test:** Fixture **Account Email** load for `tests` so the suite can arrange and assert persistence behavior. CI and developers running the suite only."))
        .await
        .expect("get")
        .expect("email");
