...
async fn seed_user(valence: &valence::Valence) -> RecordId {
    ...
    .expect("user");
    let created = User::create_used(user, valence, valence::use_!("create User in lepton-auth/tests/account_phone.rs; Valence persistence for this feature path; typed store; visible to test harness.")).await.expect("create user");
...
async fn seed_account(valence: &valence::Valence, name: &str, user: &RecordId) -> RecordId {
    ...
    .expect("account");
    let created = Account::create_used(account, valence, valence::use_!("create Account in lepton-auth/tests/account_phone.rs; Valence persistence for this feature path; typed store; visible to test harness."))
        .await
...
async fn seed_membership(
    ...
    .expect("membership");
    let created = AccountMembership::create_used(membership, valence, valence::use_!("create AccountMembership in lepton-auth/tests/account_phone.rs; Valence persistence for this feature path; typed store; visible to test harness."))
        .await
...
async fn account_phone_erase_cascades_happy() {
    ...
    assert!(AccountPhone::get_used(&bare_id_from_record(&phone_id), &valence, valence::use_!("get AccountPhone in lepton-auth/tests/account_phone.rs; Valence persistence for this feature path; typed store; visible to test harness."))
        .await
        .expect("get")
        .is_none());
}
...
async fn account_primary_phone_restrict_happy() {
    ...
    assert!(
        AccountPhone::get_used(&bare_id_from_record(&backup_id), &valence, valence::use_!("get AccountPhone in lepton-auth/tests/account_phone.rs; Valence persistence for this feature path; typed store; visible to test harness."))
            .await
            .expect("get")
            .is_none()
    );
...
async fn set_account_primary_phone_happy() {
    ...
    let acct = Account::get_used(&bare_id_from_record(&account), &valence, valence::use_!("get Account in lepton-auth/tests/account_phone.rs; Valence persistence for this feature path; typed store; visible to test harness."))
        .await
        .expect("get")
        .expect("account");
    assert_eq!(
...
async fn set_primary_phone_happy() {
    ...
    let row = User::get_used(&bare_id_from_record(&user), &valence, valence::use_!("get User in lepton-auth/tests/account_phone.rs; Valence persistence for this feature path; typed store; visible to test harness."))
        .await
        .expect("get")
        .expect("user");
    assert_eq!(
