...
async fn seed_user_with_email(
    ...
    account_created
        .get_mutable(valence, valence::use_!(r"**Test:** Fixture **this data** access in `confirm_account_status` so the suite can arrange and assert persistence. CI and developers running the suite only."))
