...
async fn seed_user_with_password(valence: &valence::Valence, password: &str) -> SeededUser {
    ...
    .expect("user");
    let created = User::create_used(user, valence, valence::use_!("create User in lepton-auth/tests/password_hash_privacy.rs; Valence persistence for this feature path; typed store; visible to test harness.")).await.expect("create user");
...
async fn password_hash_owner_read_happy_path() {
    ...
    let loaded = User::get_used(&alice.bare, &alice_v, valence::use_!("get User in lepton-auth/tests/password_hash_privacy.rs; Valence persistence for this feature path; typed store; visible to test harness."))
        .await
        .expect("get")
        .expect("alice row");
    ...
    let bob_self = User::get_used(&bob.bare, &bob_v, valence::use_!("get User in lepton-auth/tests/password_hash_privacy.rs; Valence persistence for this feature path; typed store; visible to test harness."))
        .await
        .expect("get bob")
        .expect("bob row");
...
async fn password_hash_cross_user_redacted_sad() {
    ...
    let bob_v = user_valence(&sys, &bob.bare);
    let alice_as_bob = User::get_used(&alice.bare, &bob_v, valence::use_!("get User in lepton-auth/tests/password_hash_privacy.rs; Valence persistence for this feature path; typed store; visible to test harness."))
        .await
    ...
    let alice_v = user_valence(&sys, &alice.bare);
    let bob_as_alice = User::get_used(&bob.bare, &alice_v, valence::use_!("get User in lepton-auth/tests/password_hash_privacy.rs; Valence persistence for this feature path; typed store; visible to test harness."))
        .await
...
async fn password_hash_system_read_happy_path() {
    ...
    let alice_row = User::get_used(&alice.bare, &sys, valence::use_!("get User in lepton-auth/tests/password_hash_privacy.rs; Valence persistence for this feature path; typed store; visible to test harness."))
        .await
        .expect("get alice")
        .expect("alice");
    assert_eq!(alice_row.password_hash(), Some(&alice.phc));

    let bob_row = User::get_used(&bob.bare, &sys, valence::use_!("get User in lepton-auth/tests/password_hash_privacy.rs; Valence persistence for this feature path; typed store; visible to test harness."))
        .await
        .expect("get bob")
        .expect("bob");
